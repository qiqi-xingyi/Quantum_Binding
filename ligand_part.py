...
def filter_ligand_pdb(input_pdb, output_pdb, ligand_chain):
    """
    Filter the PDB file to keep only the ligand atoms.
    Atoms with a chain identifier equal to ligand_chain are considered ligand atoms.
...
def main():
    ...
    # 3) Filter the PDB file to keep only ligand atoms (atoms in ligand_chain)
    pdb_input_path = "./data_set/data/2_benchmark_binidng_sites/1c5z/1c5z_Binding_mode.pdb"
    pdb_ligand_path = "./data_set/data/2_benchmark_binidng_sites/1c5z/1c5z_Binding_mode_ligand.pdb"
    filter_ligand_pdb(pdb_input_path, pdb_ligand_path, ligand_chain)
    print(f"Ligand-only PDB saved to: {pdb_ligand_path}")

    # 4) Build the ligand molecular system (adjust charge, spin, and basis as needed)
    builder = PDBSystemBuilder(pdb_ligand_path, charge=0, spin=0, basis="sto3g")
    ...
    print(f"Ligand active space => electrons: {active_e}, orbitals: {active_o}")
    ...
    # 9) Run the VQE to compute the ligand ground state energy
    ...
    print("Final ligand energy:", final_energy)

    # 10) Save the results
    os.makedirs("results_ligand", exist_ok=True)
    with open(os.path.join("results_ligand", "final_energy.txt"), "w") as f:
        f.write(str(final_energy) + "\n")
    with open(os.path.join("results_ligand", "energy.csv"), "w", newline="") as cf:
        writer = csv.writer(cf)
        writer.writerow(["Iter", "Energy"])
        for i, e in enumerate(energies):
            writer.writerow([i+1, e])
    with open(os.path.join("results_ligand", "params.json"), "w") as jf:
